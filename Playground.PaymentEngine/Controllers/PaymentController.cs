...
namespace Playground.PaymentEngine.Controllers {
    ...
    public class PaymentController: ControllerBase {
        ...
        [HttpPost("process")]
        public async Task<ActionResult<IEnumerable<ViewModel.ExportResponse>>> ProcessAsync([FromServices] ProcessUseCase useCase, ProcessRequest request, CancellationToken cancellationToken) => 
            await ExecuteAsync<ActionResult<IEnumerable<ViewModel.ExportResponse>>>(async () => {
                var response = await useCase.ExecuteAsync(request, cancellationToken);
                return Ok(_mapper.Map<IEnumerable<ViewModel.ExportResponse>>(response));
            });
